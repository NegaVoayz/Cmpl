...
3. **Anonymous struct identity**: Anonymous struct types (typedef'd without a tag, e.g. `typedef struct { ... } GenCtx`) are cached by AST `Type*` pointer — the same AST node always returns the same `IR_Type*`. This is critical because `clone_type_for_chain()` creates shallow copies for member chains, and `ir_struct_ast_lookup()` uses a clone fallback (matching by `name` for named types, by `members` pointer for anonymous types) to resolve field indices on clones. Named structs are cached by tag name as before.
