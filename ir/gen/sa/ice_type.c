...
/* the per-case type rules, extracted from ice_expr_type_ctx so the
 * dispatcher stays small.  Each takes the recursion context (globals
 * table + optional local symbol table) and returns the IR type or
 * NULL when it cannot be determined. */

static IR_Type*
ice_type_ident(Arena* a, AST_Node* e, HashMap* globals, GenCtx* ctx)
{
    if (ctx) {
...
static IR_Type*
ice_type_binary(Arena* a, AST_Node* e, HashMap* globals, GenCtx* ctx)
{
...
static IR_Type*
ice_type_unary(Arena* a, AST_Node* e, HashMap* globals, GenCtx* ctx)
{
...
static IR_Type*
ice_type_index(Arena* a, AST_Node* e, HashMap* globals, GenCtx* ctx)
{
...
static IR_Type*
ice_type_member(Arena* a, AST_Node* e, HashMap* globals, GenCtx* ctx)
{
...
static IR_Type*
ice_type_ternary(Arena* a, AST_Node* e, HashMap* globals, GenCtx* ctx)
{
...
ice_expr_type_ctx(Arena* a, AST_Node* e, HashMap* globals, GenCtx* ctx)
{
    ...
    case AST_IDENT:
        return ice_type_ident(a, e, globals, ctx);

    case AST_BINARY:
        return ice_type_binary(a, e, globals, ctx);

    case AST_UNARY:
        return ice_type_unary(a, e, globals, ctx);

    case AST_CAST:
        return ir_type_from_ast(a, e->body.cast.type_expr);

    case AST_INDEX:
        return ice_type_index(a, e, globals, ctx);

    case AST_MEMBER:
        return ice_type_member(a, e, globals, ctx);

    case AST_TERNARY:
        return ice_type_ternary(a, e, globals, ctx);
