/* test_struct_cache.c -- the struct dedup cache must keep its exact
 * semantics now that it is backed by hash maps.
 *
 * Growth: 100 distinct structs used as pointer locals push the name map
 * past the 70 %-load resize trigger (90 of 128) -- a lossy or crashing
 * grow breaks compilation of this file.
 * Forward-declared-then-defined: an incomplete forward-decl entry must not
 * shadow the later complete definition, or sizeof() comes back 0 and field
 * access lowers to undef (the test_struct_fwd_ref.c regression).
 * Self-referential: the in-flight entry is cached before members build, so
 * `struct SelfRef* next` terminates instead of recursing forever.
 * Anonymous typedef: two globals share one anonymous AST node -- they must
 * dedup to one IR_Type (ir_struct_ast_lookup fails on clones). */
#include <stdio.h>

struct FwdRef;
typedef struct { struct FwdRef* first; } Holder;
struct FwdRef { int x; struct FwdRef* next; };
...
/* external so it is always code-generated -- the pointer types (and thus
 * every struct above) are converted during alloca building */
int
touch(void)
{
...
int main(void)
{
    ...
    Holder h;
    struct FwdRef n1 = { 10, 0 };
    struct FwdRef n2 = { 20, 0 };

    n1.next = &n2;
    h.first = &n1;

    if (sizeof(struct FwdRef) == 0) rc |= 1;   /* incomplete shadowed def */
    if (h.first->x != 10) rc |= 2;
    if (h.first->next->x != 20) rc |= 4;

    struct SelfRef a = { 5, 0 };
    struct SelfRef b = { 7, &a };
    if (b.next->v != 5) rc |= 8;

    if (anon1.a != 3 || anon2.a != 5) rc |= 16;
