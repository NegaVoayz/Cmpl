...
#   gcc  path: gcc -> run
#
# Runs the corpus at the default -O0 and at -O1 (GVN + inline-dev), so
# optimizer-introduced wrong-code (e.g. GEP CSE with differing indices)
# is caught too.  Requires build/bootstrap/cmpl (or set CMPL=/path/to/cmpl),
# plus clang and gcc on PATH.  Prints a divergences list or "clean bill".
set -u
...
check_level()
{
    local level="$1"
    local flags=""
    [ "$level" != 0 ] && flags="-O$level"
    local PASS=0 FAIL=0 DIVERGED=""
    ...
    for f in "$CORPUS"/*.c; do
        ...
        # cmpl -> IR -> clang link
        if ! "$CMPL" $flags -emit-llvm -I"$ROOT/include" -I"$ROOT" \
    ...
    echo "diff_gcc -O$level: PASS=$PASS FAIL=$FAIL"
    if [ -n "$DIVERGED" ]; then
        printf '  diverged:%s\n' "$DIVERGED"
    else
        echo "  clean bill"
