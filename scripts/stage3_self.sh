...
P=0; F=0
for src in main.c dump_ast.c dump_ast_decl.c base/*.c tokenizer/*.c pp/*.c parser/lr/*.c parser/lr/table/*.c parser/lr/reduce/*.c parser/ll/*.c parser/ll/decl/*.c parser/parse.c ast-opt/*.c ast-opt/fold/*.c ast-opt/propagate/*.c ir/*.c ir/gen/*.c ir/dump/*.c ir-opt/*.c cuda/*.c vulkan/*.c llvm-codegen/*.c; do
    [ -f "$src" ] || continue
    b="$(echo "$src" | tr '/' '_')"
